/*
  # Coin Balance System Optimization
...
BEGIN
    ...
    LOOP
        ...
        IF migrated_users % 100 = 0 THEN
            RAISE NOTICE 'Migrated % users, % transactions...', migrated_users, migrated_transactions;
        END IF;
    END LOOP;
...
BEGIN
    ...
    IF (migration_result->>'success')::boolean THEN
        ...
        RAISE NOTICE '   - Users migrated: %', migration_result->>'migrated_users';
        RAISE NOTICE '   - Transactions migrated: %', migration_result->>'migrated_transactions';
        RAISE NOTICE '   - Duration: % seconds', ROUND((migration_result->>'duration_seconds')::decimal, 2);
    ELSE
    ...
    IF (validation_result->>'success')::boolean THEN
        ...
        RAISE NOTICE '   - Users checked: %', validation_result->>'total_users_checked';
        RAISE NOTICE '   - Integrity status: %', validation_result->>'integrity_status';
    ELSE
        RAISE WARNING '⚠️  Data integrity validation FAILED!';
        RAISE WARNING '   - Mismatched users: %', validation_result->>'mismatched_users';
        RAISE WARNING '   - Total users checked: %', validation_result->>'total_users_checked';
    END IF;
    ...
    RAISE NOTICE '   - Storage reduction: %% (% bytes saved)', 
        metrics_result->>'storage_reduction_percent',
        (metrics_result->>'old_system_size_bytes')::bigint - (metrics_result->>'new_system_size_bytes')::bigint;
    RAISE NOTICE '   - Average transactions per user: %', metrics_result->>'avg_transactions_per_user';
    RAISE NOTICE '   - Performance improvement: %', metrics_result->>'performance_improvement';
    ...
    RAISE NOTICE '   ✓ 80-90%% reduction in database storage usage';
