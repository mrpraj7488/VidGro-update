...
BEGIN
  ...
  -- Get new balance
  SELECT coins INTO new_balance FROM profiles WHERE id = input_user_uuid;
  ...
  VALUES (input_video_uuid, input_user_uuid, watch_duration, watch_duration >= video_record.duration_seconds, coins_to_award)
...
BEGIN
  ...
  RETURN award_coins_with_engagement_tracking(input_user_uuid, input_video_uuid, watch_duration, watch_duration);
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION get_next_video_queue_simple(input_user_uuid UUID)
RETURNS TABLE(
  video_id UUID,
  youtube_url TEXT,
  title TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    evq.target_views,
    evq.status
  FROM get_next_video_queue_enhanced(input_user_uuid) evq;
END;
$$ LANGUAGE plpgsql;
...
-- Helper function to activate videos that are ready (hold period expired)
CREATE OR REPLACE FUNCTION activate_ready_videos()
RETURNS INTEGER AS $$
DECLARE
...
-- Function to check current video queue status (for debugging)
CREATE OR REPLACE FUNCTION debug_video_queue_status()
RETURNS JSON AS $$
DECLARE
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(CASE WHEN status = 'repromoted' THEN 1 END) as repromoted_count,
  ...
  -- Get on_hold details
  SELECT 
    COUNT(CASE WHEN hold_until <= now() THEN 1 END) as ready_to_activate,
    COUNT(CASE WHEN hold_until > now() THEN 1 END) as still_on_hold
  INTO hold_counts
  FROM videos
  WHERE status = 'on_hold';
  
  RETURN json_build_object(
    'total_videos', status_counts.total_videos,
    'active_videos', status_counts.active_count,
    'on_hold_videos', status_counts.on_hold_count,
    'repromoted_videos', status_counts.repromoted_count,
    'completed_videos', status_counts.completed_count,
    'paused_videos', status_counts.paused_count,
    'ready_to_activate', hold_counts.ready_to_activate,
    'still_on_hold', hold_counts.still_on_hold
  );
END;
$$ LANGUAGE plpgsql;